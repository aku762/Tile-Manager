...
| `featured` | boolean | `true` makes this a featured section — its tiles are sourced by `featured` order, not by section assignment. Only one section should have this flag. |
...
- **Sections** — add, edit, delete, and drag-to-reorder sections. Each section has a hide/show toggle. Sections marked `featured` show a badge and cannot be deleted.
...
Mark one section with `"featured": true` in `tiles.json` (or leave it set via the admin). That section ignores its own tile assignments and instead displays all tiles that have a `featured` order number, sorted by that number. To feature a tile, click the ☆ in the admin — it becomes ★ and is assigned the next available position. To reorder, select the featured section from the section dropdown and drag tiles into the desired order.
