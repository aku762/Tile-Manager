...
function renderTile(tile, statusMap) {
    ...
    const audioHTML = tile.audio
        ? `<div class="tile-audio${showImg ? ' tile-audio-overlay' : ''}" data-src="${tile.audio}" data-name="${tile.name}" data-artist="${tile.artist || ''}" data-album="${tile.album || ''}" data-image="${tile.image || ''}" data-image-format="${imgFolder}"><button class="audio-btn" onclick="audioPlay(this)">▶</button><div class="audio-bar" onclick="audioSeek(event,this)"><div class="audio-prog"></div></div><span class="audio-time">0:00</span><button class="audio-btn" onclick="audioMute(this)">🔊</button></div>`
        : '';
    ...
    if (showImg) {
        ...
        if (tile.image) {
            ...
            img.src      = `images/${imgFolder}/${tile.image}`;
            img.alt      = tile.name;
            img.loading  = 'lazy';
            img.decoding = 'async';
